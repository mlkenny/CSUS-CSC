...
public class Sorting {
	...
	public static void MergeSort(int data[], int lo, int hi)
	{
		...
		MergeSort(data, lo, mid); // Sort left of middle value, including middle value.
		MergeSort(data, mid + 1, hi); // Sort right of middle value.
		Merge(data, lo, mid, hi); // Merge left and right sorts.
	}

	public static void Merge(int data[], int lo, int mid, int hi) {
		int n1 = mid - lo + 1; // Size of left array.
		int n2 = hi - mid; // Size of right array.

		int[] left = new int[n1 + 1];
		int[] right = new int[n2 + 1];
		
		// Load arrays up with values from data.
		for (int i = 0; i < n1; i++) {
			left[i] = data[lo + i];
		}
		for (int j = 0; j < n2; j++) {
		...
		int MAX = Math.max(left[n1], right[n2]);
		left[n1] = MAX;
		right[n2] = MAX;

		// Initialize index trackers.
		int leftIndex = 0, rightIndex = 0;
		int mainIndex = lo; // Important to start data's index at lo index.

		// Unload and evaluate values of left and right back into data.
	...
	public static void QuickSort(int data[], int lo, int hi, int depth)
	{
		...
		if (depth > max_depth) {
			max_depth = depth; // Tracks maximum recursion depth.
		}

		// Insertion Sort Optimization.
		if (hi - lo + 1 < 40) {
			InsertionSort(data, hi - lo);
		}

		int mid = Partition(data, lo, hi);
		QuickSort(data, lo, mid - 1, depth + 1); // Sort left side of middle. Increment depth each call.
		QuickSort(data, mid + 1, hi, depth + 1); // Sort right side of middle. Increment depth each call.
	}

	public static int Partition(int data[], int lo, int hi) {
		// Randomization Pivot Optimization.
		Random rand = new Random(); // Needs import of random library.
		int randomIndex = rand.nextInt(hi - lo + 1) + lo; // Select random index based off lo and hi.
		swap(hi, randomIndex, data); // Swap random value to end of array.
	...
	public static int Median_Of_Three(int data[], int lo, int hi) {
	...
	public static int Select(int data[], int lo, int hi, int i) {
		if (lo == hi) {
			return lo;
		}
	...
	public static int Partition_Median(int data[], int lo, int hi) {
		int pivot = data[hi]; // Choose last element of data as pivot.
		int trace = lo - 1;
		for (int i = lo; i < hi; i++) { // Loop from lo to hi.
			if (data[i] <= pivot) {
	...
	{
		Arrays.sort(data); // Standard library sort.
	}
	/*****************************************************************************/
